...
```{r}
gabon <- read_csv("IvindoData_DryadVersion.csv") %>%
  clean_names()
```
...
```{r}
ggplot(data=gabon,
       mapping=aes(x=distance, y=land_use))+
  geom_boxplot()+
  labs(title="Distance v Land Use")
```
...
```{r}
ggplot(data=gabon,
       mapping=aes(x=veg_rich, y=land_use))+
  geom_boxplot()+
  labs(title="Vegetation Richness vs. Land Use")
```
...
ggplot(data=gabon,
  ...
  geom_boxplot()+
  labs(title="Land Use vs Species Diversity (all)")
```
...
ggplot(data=gabon,
  ...
  geom_point()+
  labs(title="Vegetation Richness vs Species Richness (mammals)")
```
...
```{r}
filter(gabon, veg_rich>=15)
  
```

```{r}
filter(gabon, between(veg_dbh, 50, 60)) 
```
...
```{r}
#filter(gabon, veg_rich <= 10)
#filter(gabon, rich_mammal_species==10)
#arrange(desc(veg_rich))
#is this code not supposed to run?
```
