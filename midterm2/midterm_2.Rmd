...
polar_bears %>%
  ...
  geom_col(alpha=0.6)+
  labs(title="Number of Polar Bears Sampled by Sex",
       x="Sex",
...
bear_id <- polar_bears$bear_id
year <- polar_bears$year
month <- polar_bears$month
day <- polar_bears$day
popn <- polar_bears$popn
sex <- polar_bears$sex
age_class <- polar_bears$age_class
mating <- polar_bears$mating
denning <- polar_bears$denning
lat <- polar_bears$lat
long <- polar_bears$long
urea_nitrogen <- polar_bears$urea_nitrogen
creatinine <- polar_bears$creatinine
```
```{r}
polar_bears <- tibble(bear_id,    year, month,  day,  popn, sex, age_class, mating, denning, long, urea_nitrogen, creatinine, un_creat_ratio)
```
```{r}
polar_bears
```
...
```{r, eval=F, echo=T}
ui <- dashboardPage(
  dashboardHeader(title="The Range of Urea Nitrogen/Creatinine Ratio By Population"),
  dashboardSidebar(
    selectInput("y",
              "Select Population",
              choices = c("popn"),
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    elephants %>% 
      filter(un_creat_ratio!="NA") %>% 
      ggplot(aes(x = un_creat_ratio,
                 y = .data[[input$y]],
                 fill = popn))+
      geom_boxplot(alpha=0.75)+
      labs(title="Range of Variables by Population",
           x="Population",
           fill="Population")+
      theme_minimal()
  })
