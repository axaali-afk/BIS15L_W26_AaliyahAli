...
Link to repository: https://github.com/axaali-afk/BIS15L_W26_AaliyahAli
...
```{r}
anolis <- read_csv("anolis_dat.csv")
```
...
anolis %>%
  ggplot(data=anolis,
         mapping=aes(x=island, y=species))+
  geom_col(mapping=aes(fill=island))+
  labs(title="Island vs Species")
```
...
anolis %>%
  ggplot(data=anolis,
         mapping=aes(x=habitat, y=species))+
  geom_col(mapping=aes(fill=habitat))+
  labs(title="Habitat vs Species")
```
...
anolis %>%
  ...
  geom_boxplot()+
  labs(title="Habitat vs Hind Limb Length (mm)")
```
...
species <- anolis$species
habitat <- anolis$habitat
hindlimb_length_mm <- anolis$hindlimb_length_mm
tail_length_mm <- anolis$tail_length_mm
body_length_mm <- anolis$body_length_mm
toepad_lamellae_count <- anolis$toepad_lamellae_count
island <- anolis$island

```
```{r}
anolis <- tibble(species, habitat, hindlimb_length_mm, tail_length_mm, body_length_mm, toepad_lamellae_count, island, ratio_of_hindlimb_to_body, ratio_of_tail_to_body, )
...
anolis %>%
  ...
  geom_boxplot()+
  labs(title="Habitat vs Hind Limb To Body Ratio")
```
...
#There is a positive relationship between body length and tail length. Lizards in the grass brush habitat have longer tails, but have smaller bodies.
```
...
anolis %>%
  ...
  geom_col(mapping=aes(fill=island))+
  labs(title="Island vs. Toepad Lamellae Counts")
```
