...
```{r message=FALSE, warning=FALSE}
spider_records <- read_csv("spider_records.csv") %>%
  clean_names()
spider_records
```

```{r, message = FALSE}
spider_location <- read_csv("spiders_locations.csv") %>%
  clean_names()
spider_location
```
...
```{r}
left_join(spider_records, spider_location, by="accession")
```

```{r}
spider <- left_join(spider_records, spider_location, by="accession")
spider
```
...
```{r}
write.csv(spider, file="spider.csv", row.names = T)
```
