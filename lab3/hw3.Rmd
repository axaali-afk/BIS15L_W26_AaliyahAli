...
```{r}
my_vector_name <- c("Mount Everest", "K2", "Kangchenjunga", "Lhotse", "Makalu")
my_vector_height_ft <- c(29032, 28251, 28169, 27940, 27838)
my_vector_height_m <- c(8849, 8611, 8586, 8516, 8485)
```

**2. Combine these vectors into a data frame called `mountains`.**
```{r}
mountains <- tibble(my_vector_name, my_vector_height_ft, my_vector_height_m)
```
```{r}
mountains
```
...
```{r}
mean(mountains$my_vector_height_ft)
```
...
```{r}
mountains <- tibble(my_vector_name, my_vector_height_ft, my_vector_height_m, first_climbed)
```
...
```{r}
summits <- c(12884, 800, 180, 1000, 499)
```
```{r}
mountains <- tibble(my_vector_name, my_vector_height_ft, my_vector_height_m, first_climbed, summits)
```
...
```{r}
fatalities <- c(344, 100, 52, 22, 300)
```
...
```
```{r}
mountains <- tibble(my_vector_name, my_vector_height_ft, my_vector_height_m, first_climbed, summits, fatalities, fatality_rate)
```
...
```{r}
mountains <- read_csv("mountains_data")
```
