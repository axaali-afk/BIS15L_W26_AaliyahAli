...
heartrate_long %>%
  ...
  ggplot(data=heartrate_long,
         mapping=aes(x=drug, y=heartrate))+
  geom_col()
```
...
_varaible names are actually data_
3. Use `pivot_longer()` to make the data tidy.  
```{r}
relig_income %>%
  pivot_longer(-religion, #don't move
               names_to = "income", 
               values_to= "total" )#give new column name
#move the count to a new column
```
